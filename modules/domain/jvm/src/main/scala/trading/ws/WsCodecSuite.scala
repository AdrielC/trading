  ...
  test("roundtrip conversion of WsIn messages") {
    IO.pure {
      (jsonDecode[List[WsIn]](in), jsonDecode[List[WsIn]](in2)) match {
        case (Right(res1), Right(res2)) => expect.same(res1.asJson.spaces2, in) && expect.same(res2.asJson.noSpaces, in2)
        case _ => failure("fail to decode")
      }
    }
  }
  ...
  test("roundtrip conversion of WsOut messages") {
    IO.pure {
      jsonDecode[List[WsOut]](out) match {
        case Right(res) => expect.same(res.asJson.spaces2, out)
