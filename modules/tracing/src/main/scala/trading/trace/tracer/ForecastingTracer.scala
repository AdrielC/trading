...
object ForecastingTracer:
  def make[F[_]: MonadCancelThrow : Sync](
    ...
    def trace(cmd: ForecastCommand, evt: Either[AuthorEvent, ForecastEvent]): F[Unit] = Sync[F].defer({
    ...
    }).recoverWith({ case e: Throwable => Sync[F].delay(println(f"Error in creating Trace: ${e}")) })


    /* the following two methods are not used but this is how it could look if it followed the trading design */
    def command(cmd: ForecastCommand): F[Kernel] = Sync[F].defer({
    ...
    def event(kernel: Kernel, evt: Either[AuthorEvent, ForecastEvent]): F[Unit] =  Sync[F].defer({
      val cid       = evt.fold(_.cid, _.cid)
      val createdAt = evt.fold(_.createdAt, _.createdAt)
